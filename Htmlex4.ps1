...
$HTML = ConvertTo-Html -Body "$Services $Process" -Head "<style> body {background-color: lightblue; } table {background-color: white;  top: 0px;  display: inline-block; float:left;margin:5px;padding: 5px; border: 1px solid black} tr:nth-child(odd) {background-color: lightgray} </style>"

#Output to a file
$HTML | Out-File C:\temp\Test.html

Invoke-Item -Path C:\temp\Test.html